...
Kubernetes state is declared in `kelvinSeamount/zen-gitops`, not controlled by imperative `kubectl apply` commands in CI. ArgoCD watches that repo and reconciles the cluster toward the declared state. This means every deployment has a git history, an author, a timestamp, and is rollback-able by reverting a commit.
